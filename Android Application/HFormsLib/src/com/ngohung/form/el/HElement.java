...
public abstract class HElement { // base element for all elements
	...
	private HDataStore dataStore = null; // defined the data store use to
											// save/load value
	protected String key; // unique key for this element if we need to store to
							// SharePreferences
	protected String label; // label for the element
	protected String value; // value for this element e.g value for EditText
	protected String hint = HConstants.BLANK; // placeholder for edittext if
												// necessary
	...
	protected String requireMsg; // if this field is required, use this message
									// to display
	protected int maxLength = HConstants.NOT_SPECIFIED; // max length for
														// elements, do
														// maxlength
	protected int keyboardType = HConstants.NOT_SPECIFIED; // for numeric fields
															// field, set this
															// to numeric
															// keyboard. Only
															// applicable to
	...
	public abstract void loadValueForUI(View v); // load value of this element
													// into its UI e.g. load
													// value into EditText
													// textbox

	public abstract void saveValueFromUI(View v); // when user has entered text
													// in the UI, we need to
													// store the changes back to
													// this element

	public abstract ValidationStatus doValidationForUI(View v); // do validation
																// and display
																// errors/success
																// in the UI if
																// necessary.
	...
	// if it is need to limit max length in the UI e.g. max number of characters
	// in EditText
	...
	// if there is at least 1 validator or the field is required, must return a
		...
		if (required) {
			...
			if (value == null || value.length() == 0) {
				...
				if (requireMsg != null)
					...
					valStatus.setMsg(String.format(DEFAULT_REQUIRED_MSG,
							this.label.toLowerCase(Locale.ENGLISH))); // default message for
														// required msg
				}
				return valStatus;
			}
			...
			if (value == null || value.length() == 0)
				valStatus = null; // optional field, we will mark this
									// validation as optional by passing null
		}
