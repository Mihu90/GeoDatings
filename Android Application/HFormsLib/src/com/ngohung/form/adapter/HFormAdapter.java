...
public class HFormAdapter extends BaseAdapter implements HFormAdapterInterface {
	...
	public final static String REQUIRED_LABEL_FMT = "<font color='red'>*</font> %s"; // for
																						// required
																						// field
																						// display
																						// *
																						...
																						// red
																						// color
	public final static String STD_LABEL_FMT = "%s"; // if we need to give
														// spacing to label, we
														// can edit this
	...
	public int getViewTypeCount() // return number of different views we have.
									// This is used for reusing the convertview
	{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			case HElementType.TEXT_EL: {
				convertView = inflater.inflate(R.layout.form_text_row_item,
						parent, false);
			}
				break;
			...
			case HElementType.TEXT_ENTRY_EL: {
				...
				convertView = inflater.inflate(
						R.layout.form_text_entry_row_item, parent, false);
			}
				break;
			...
			case HElementType.LIST_EL: {
				convertView = inflater.inflate(R.layout.form_picker_row_item,
						parent, false);

				final View pickerBtnView = convertView
						.findViewById(R.id.btn_picker);
			...
			holder.labelTV = (TextView) convertView
		...
		if (holder.labelTV != null && curItem.getLabel() != null) {
			...
			if (curItem.isRequired()) {
				holder.labelTV.setText(Html.fromHtml(String.format(
						REQUIRED_LABEL_FMT, curItem.getLabel())));
			} else {
				holder.labelTV.setText(String.format(STD_LABEL_FMT,
						curItem.getLabel()));
			}
		}

		// Log.i("HFormAdapter", "element key = " + curItem.getKey() +
		// ", hidden: " + curItem.isHidden() + ", valueTV: " + holder.valueTV +
		// ", eltype: " + curItem.getElType());

		// handle keyboard events
		// let our models (HElement) handle the events, update the behind
		// models, and validate automatically
		if (holder.valueTV != null && curItem.getValue() != null) {
			...
			if (curItem.isReadonly())
				holder.valueTV.setFocusable(false);
			
			OnClickListener listener = curItem.getOnClickListener();
			if (listener != null)
				holder.valueTV.setOnClickListener(listener);
			
			curItem.setEditText(holder.valueTV);

			if (curItem instanceof HTextEntryElement) {
				HTextEntryElement textEntryEl = (HTextEntryElement) curItem;
				holder.valueTV.setOnFocusChangeListener(textEntryEl); // when
																		// element
																		// lose
																		// focus,
																		// it
			...
			holder.valueTV
					.setImeOptions((position == rootEl
							.getLastInputKeyboardElPosition()) ? EditorInfo.IME_ACTION_DONE
		...
		case HElementType.MULTI_LINES_TEXT_ENTRY_EL: {
			...
			if (!curItem.hasSpecifyKeyboardType())
				holder.valueTV.setInputType(InputType.TYPE_CLASS_TEXT
		...
		case HElementType.NUMERIC_ENTRY_EL: {
			holder.valueTV.setKeyListener(new HNumericKeyboardKeyListener()); // force
																				// numeric
																				// input
		}
			break;
		...
		// case HElementType.LIST_EL: // list element will generate a single
		// choice listview
		// {
		...
		convertView
				.setVisibility(curItem.isHidden() ? View.GONE : View.VISIBLE);
	...
	// we need this view sometimes as we need to display validation in the view
	// e.g. the ! error icon at the right for button/edittext
	public View getViewForValidation(View convertView, int position) {
		...
		if (convertView != null && convertView.getTag() != null) {
			...
			case HElementType.TEXT_EL: // no need to do anything for text
										// element
				return null;
	...
	public View getHeaderView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(
					R.layout.default_form_section_header, parent, false);
			holder.text = (TextView) convertView
					.findViewById(R.id.sectionHeaderText);
