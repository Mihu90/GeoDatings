...
public abstract class HBaseFormActivity extends ActionBarActivity {

	protected HRootElement rootElement; // the element represent the form to
										// build
	protected HFormAdapter formAdapter;

	protected LinearLayout formContainerView;
	protected TextView instructionTextView; // our instruction if there is e.g.
											// * denotes mandatory
	protected List<View> formViews = new ArrayList<View>(); // list of views
															// that are used for
															// validation e.g.
															// EditText for text
															// entry elements,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// get back the view
		formContainerView = (LinearLayout) this
				.findViewById(R.id.formContainer);
		instructionTextView = (TextView) formContainerView
	...
	// when user left the form by closing or clicking on the Back button, we
	...
	protected void buildForm(HFormAdapter adapter, LinearLayout containerView,
			List<View> validationViews) {
		...
		for (int position = 0; position < numOfItems; position++) {
			...
			if (headerId != lastHeaderId) {
				...
				View headerView = adapter.getHeaderView(position, null,
						containerView);
			...
			View validationView = adapter.getViewForValidation(convertView,
					position);
	...
	// for list element, after list selection, validation should be triggered
	// immediately
